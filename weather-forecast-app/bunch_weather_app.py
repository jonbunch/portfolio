...
def get_user_location():
    ...
    user_loc = ''
    while user_loc != 'quit':
        user_loc = input('Please enter a five digit zip code OR city, state, and country (separated by commas) OR '
                         'enter x to exit:')
        if user_loc.lower() == 'x':
            return 'quit'
        elif user_loc.isdigit():
            if len(user_loc) != 5:
            ...
            clean_loc_split = [s.strip().lower() for s in user_loc_split if s.isalpha()]
            if len(clean_loc_split) != 3:
                print(entry_error_name)
                continue
            else:
                user_loc = ','.join(clean_loc_split)
            return user_loc
